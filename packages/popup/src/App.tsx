...
function App() {
  ...
  return (
    <>
      <div className="card">
        <Menu appName={t('apps.name')} version={version} />
        <button onClick={() => setCount((count) => count + 1)}>
          count is {count}
        </button>
        <button onClick={() => goToSomePath('/settings')}>{t('apps.settings')}</button>
        <button onClick={() => goToSomePath('/about')}>{t('apps.about')}</button>
      </div>
    </>
  )
}
